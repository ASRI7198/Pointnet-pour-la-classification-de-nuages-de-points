...
def sample_cylinder(npts=2048):
    ...
    u[:, 0] = np.sqrt(u[:, 0])
    x1 = np.concatenate((-l / 2 * np.ones(nptscirc), u[:, 0] * r * np.cos(2 * np.pi * u[:, 1]),
                         u[:, 0] * r * np.sin(2 * np.pi * u[:, 1]))).reshape((nptscirc, 3), order='F')

    u = np.random.rand(nptscirc, 2)
    u[:, 0] = np.sqrt(u[:, 0])
    x2 = np.concatenate((l / 2 * np.ones(nptscirc), u[:, 0] * r * np.cos(2 * np.pi * u[:, 1]),
                         u[:, 0] * r * np.sin(2 * np.pi * u[:, 1]))).reshape((nptscirc, 3), order='F')

    u = np.random.rand(nptscyl, 2)
    x3 = np.concatenate(
        (-l / 2 + l * u[:, 0], r * np.cos(2 * np.pi * u[:, 1]), r * np.sin(2 * np.pi * u[:, 1]))).reshape((nptscyl, 3),
                                                                                                          order='F')
